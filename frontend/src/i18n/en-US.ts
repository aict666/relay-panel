...
export const enUS: Dict = {
  ...
  dashboardNetworkTrend: 'Machine Network & Connections',
  dashboardNetworkTrendHint: 'Bandwidth comes from each online node’s monitored NIC and includes SSH, system updates, and other non-RelayPanel traffic. Bandwidth and connections retain each bucket’s peak so short spikes remain visible in longer ranges.',
  dashboardUpload: 'Upload',
  dashboardDownload: 'Download',
  dashboardConnectionsPeak: 'Peak connections',
  dashboardHistoryCollecting: 'Collecting history; the first point appears in about one minute',
  dashboardHistoryFailed: 'Could not load history; retrying in 60 seconds',
  dashboardLiveFailed: 'Could not load live group or rule data; retrying in 10 seconds',
  dashboardGroupHealth: 'Group Health',
  dashboardHealthPartial: 'Partial',
  dashboardHealthUnreported: 'Not reporting',
  dashboardNoGroups: 'No device groups',
  dashboardGroupRateTop: 'Live Bandwidth by Group · Top 8',
  dashboardGroupRateHint: 'Current monitored-NIC rate from online nodes; this is not RelayPanel forwarded traffic.',
  dashboardNoLiveTraffic: 'No live bandwidth to display',
  dashboardRuleTrafficTop: 'Cumulative Rule Traffic · Top 5',
  dashboardRuleTrafficHint: 'Actual bytes forwarded by RelayPanel since the last traffic reset.',
  dashboardRuleTrafficSinceReset: 'Since the last traffic reset',
  dashboardNoRuleTraffic: 'No rules have transferred traffic yet',
  dashboardViewAll: 'View all',
  dashboardHistoryRefresh: 'History refreshes every 60 seconds',
  ...
  lbWeightedDesc: 'Weighted: distributes new connections by target weight and skips circuit-open targets.',
  lbLeastLatencyDesc: 'Least latency: selects by active-probe latency and packet-loss score.',
  lbLeastConnectionsDesc: 'Least connections: selects the target with the fewest active TCP connections / UDP sessions.',
  ...
  lbFailover: 'Failover',
  lbWeighted: 'Weighted',
  lbLeastLatency: 'Least latency',
  lbLeastConnections: 'Least connections',
  uotAutoHint: 'The UDP component of UDP and TCP+UDP chains supports authenticated UOT on a dedicated port; TCP supports warm-connection 0-RTT via TFO. Native paths remain active until each feature is explicitly enabled after every node is upgraded.',
  capacityScore: 'Capacity score',
  predictedSpareConnections: 'Predicted spare connections',
  anomalyStatus: 'Anomaly detection',
  anomalyDetected: 'Abnormal surge detected',
  normal: 'Normal',
  ...
  inboundOutbound: 'Inbound + outbound (dual role)',
  ...
  modePresetTunnel: 'Preset Tunnel',
  chainHops: 'Chain hops',
  chainHopsHint: 'Ordered groups: entry → mid… → exit (at least 2). Mid/exit may use "out" or "inbound + outbound"; set connect_host.',
  addHop: 'Add hop',
  hopEntry: 'Entry',
  hopMid: 'Mid',
  hopExit: 'Exit',
  ...
  tunnelManagementHint: 'A tunnel presets only the ingress, relay, and egress route. Targets, balancing, limits, and public ingress ports remain rule-specific.',
  tunnelAccess: 'Access',
  tunnelUserSharing: 'Allow ordinary users',
  tunnelUserSharingHint: 'When enabled, only users whose plan grants the tunnel ingress line may select it. No separate per-user tunnel assignment is needed. Administrators always have access.',
  tunnelSharedWithUsers: 'Shared',
  tunnelAdminOnly: 'Admin only',
  tunnelUnshareWarning: 'Turning sharing off immediately stops ordinary-user rules bound to this tunnel; they recover automatically when sharing is restored. {count} rules are currently bound.',
  tunnelPath: 'Tunnel Route',
  sharedTunnelPorts: 'Shared Internal Ports',
  boundRuleCount: 'Bound Rules',
  tunnelHopCountHint: 'Use 2–8 unique groups. Every group after ingress must have a connect address.',
  tunnelDuplicateHop: 'Device groups in a tunnel route must be unique',
  autoPort: 'Auto Port',
  fixedPort: 'Fixed Port',
  firewallAllowlist: 'Firewall Allowlist',
  firewallAllowlistHint: 'Allow these TCP ports on the listed groups. TCP, UDP, and TCP+UDP rules share the same internal TCP ports.',
  tunnelEntryChangeWarning: 'This tunnel has {count} bound rules. Changing ingress revalidates every owner; the ingress billing rate changes from {oldRate} to {newRate}.',
  tunnelSelectHint: 'Select a complete route preset by an administrator',
  tunnelPortsReused: 'Internal ports are reused; this rule still owns its public ingress port.',
  ...
  diagnoseScopeHint: 'Diagnosis shows the TCP probe each relay-node on the path runs: whether its listener is up, and node→next-hop/final-target TCP connectivity and connect time. Multi-hop chains probe entry→next hop, mid→next hop, and exit→final target separately. Latency is the node→target TCP handshake time, NOT client end-to-end latency. Pure-UDP rules are not supported; TCP+UDP rules diagnose TCP only.',
  diagnoseUdpUnsupported: 'UDP is not supported for diagnosis',
  diagnoseIngress: 'Path Node Diagnosis',
  diagnoseNoNodes: 'No dispatchable online node on this rule\'s path.',
  ...
  siteName: 'Site name',
  siteNameHint: 'Shown in the sidebar, login page, and browser title.',
  siteNameRequired: 'Please enter a site name.',
  siteNameTooLong: 'Site name must not exceed 64 characters.',
