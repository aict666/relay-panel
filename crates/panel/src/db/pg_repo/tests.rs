...
async fn pg_delete_user_cascade_removes_rules_groups_profiles_and_user() {
    ...
    .unwrap();
    sqlx::query(
        "INSERT INTO tunnels(name,enabled,shared,uid) VALUES('alice-preset',TRUE,FALSE,$1)",
    )
    .bind(uid)
    .execute(&db.pool)
    .await
...
#[tokio::test]
async fn pg_user_delete_cascade_reports_legacy_group_used_by_tunnel() {
    let Some(db) = repo("user_tunnel_conflict").await else {
        return;
    };
    ...
    sqlx::query(
        "INSERT INTO device_groups(id,name,group_type,connect_host,token,uid) VALUES \
         (810,'legacy-entry','in','entry.example','legacy-entry-token',$1), \
    ...
    .await
    .unwrap();
    // Reproduce historical data directly: current write paths intentionally
    // reject regular-user-owned tunnel entries.
    let tunnel_id: i64 = sqlx::query_scalar(
        "INSERT INTO tunnels(name,enabled,shared,uid) \
         VALUES('legacy-owner-path',TRUE,FALSE,1) RETURNING id",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    sqlx::query(
        "INSERT INTO tunnel_hops(tunnel_id,position,device_group_id,listen_port) \
         VALUES($1,0,810,NULL),($1,1,811,38110)",
    )
    .bind(tunnel_id)
    .execute(&db.pool)
    .await
...
#[tokio::test]
async fn pg_user_delete_cascade_reports_other_legacy_group_references() {
    let Some(db) = repo("user_group_ref_conflict").await else {
        return;
    };
    ...
    sqlx::query(
        "INSERT INTO device_groups(id,name,group_type,connect_host,token,uid,fallback_group) VALUES \
         (820,'legacy-owned','in','legacy.example','legacy-owned-token',$1,NULL), \
...
#[tokio::test]
async fn pg_user_delete_cascade_reports_active_cross_owner_rule_leases() {
    let Some(db) = repo("user_active_group_lease_conflict").await else {
        return;
    };
    ...
    sqlx::query(
        "INSERT INTO device_groups(id,name,group_type,connect_host,token,uid) VALUES \
         (822,'retired-entry','in','old-entry.example','retired-entry-token',$1), \
         (823,'route-transition','out','old-exit.example','route-transition-token',$1), \
    ...
    let tunnel_id: i64 = sqlx::query_scalar(
        "INSERT INTO tunnels(name,enabled,shared,uid) \
         VALUES('lease-carrier',TRUE,FALSE,1) RETURNING id",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    ...
    sqlx::query(
        "INSERT INTO forward_rule_retired_entries(rule_id,tunnel_id,device_group_id,expires_at) \
         VALUES(822,$1,822,EXTRACT(EPOCH FROM now())::BIGINT+600)",
    )
    .bind(tunnel_id)
    .execute(&db.pool)
    .await
    ...
    sqlx::query(
        "INSERT INTO forward_rule_route_transitions \
         ...
         VALUES(822,823,38223,'tcp',EXTRACT(EPOCH FROM now())::BIGINT+600)",
    )
    .execute(&db.pool)
    .await
    .unwrap();
...
#[tokio::test]
async fn pg_user_delete_cascade_reports_owned_tunnel_retired_by_other_user() {
    let Some(db) = repo("user_owned_tunnel_conflict").await else {
        return;
    };
    ...
    let tunnel_id: i64 = sqlx::query_scalar(
        "INSERT INTO tunnels(name,enabled,shared,uid) \
         VALUES('legacy-user-preset',TRUE,TRUE,$1) RETURNING id",
    )
    .bind(uid)
    .fetch_one(&db.pool)
    .await
    ...
    sqlx::query(
        "INSERT INTO forward_rule_retired_entries(rule_id,tunnel_id,device_group_id,expires_at) \
         VALUES(830,$1,830,EXTRACT(EPOCH FROM now())::BIGINT+600)",
    )
    .bind(tunnel_id)
    .execute(&db.pool)
    .await
...
#[tokio::test]
async fn pg_active_rule_leases_protect_group_mutation_until_expiry() {
    let Some(db) = repo("active_rule_lease_group_guard").await else {
        return;
    };
    sqlx::query(
        "INSERT INTO device_groups(id,name,group_type,connect_host,token,uid) VALUES \
    ...
    let tunnel_id: i64 = sqlx::query_scalar(
        "INSERT INTO tunnels(name,enabled,shared,uid) \
         VALUES('old-entry-carrier',TRUE,FALSE,1) RETURNING id",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    ...
    sqlx::query(
        "INSERT INTO forward_rule_retired_entries(rule_id,tunnel_id,device_group_id,expires_at) \
         VALUES(840,$1,840,EXTRACT(EPOCH FROM now())::BIGINT+600)",
    )
    .bind(tunnel_id)
    .execute(&db.pool)
    .await
    ...
    sqlx::query(
        "INSERT INTO forward_rule_route_transitions \
         (rule_id,device_group_id,listen_port,protocol,expires_at) \
         VALUES(840,841,38401,'tcp',EXTRACT(EPOCH FROM now())::BIGINT+600)",
    )
    .execute(&db.pool)
    .await
    .unwrap();
    ...
    sqlx::query(
        "UPDATE forward_rule_retired_entries \
    ...
    .unwrap();
    sqlx::query(
        "UPDATE forward_rule_route_transitions \
         SET expires_at=EXTRACT(EPOCH FROM now())::BIGINT-1",
    )
    .execute(&db.pool)
    .await
...
#[tokio::test]
async fn pg_upgrade_from_revision_28_adds_credential_revision() {
    let Some(db) = repo("upgrade_rev28_credential_revision").await else {
        return;
    };
    sqlx::query("ALTER TABLE device_groups DROP COLUMN credential_revision")
        ...
        .await
        .unwrap();
    sqlx::query("DELETE FROM schema_version WHERE version >= 29")
        .execute(&db.pool)
        .await
        .unwrap();

    // Production startup applies the idempotent baseline first. CREATE TABLE IF
    // NOT EXISTS cannot add a column to the existing table, so revision 29 must
    // remain reachable through PG_SCHEMA_VERSION.
...
async fn pg_group_insert_then_find_by_token_round_trip() {
    ...
    sqlx::query("INSERT INTO users(id,username,password,admin) VALUES(2,'legacy-owner','x',FALSE)")
        .execute(&db.pool)
        .await
        .unwrap();
    sqlx::query(
        "INSERT INTO device_groups(id,name,group_type,token,connect_host,uid) \
         VALUES(999,'legacy-node','out','tok-999','192.0.2.99',2)",
    )
    .execute(&db.pool)
    .await
    .unwrap();
    assert!(
        ...
        "historical regular-owned node credentials must fail closed (PG)"
    );
    assert!(!db
...
/// PG parity: every runtime-config query fails closed after entry access loss.
#[tokio::test]
async fn pg_runtime_config_requires_current_entry_group_authorization() {
    ...
    let rule_id: i64 = sqlx::query_scalar(
        ...
        VALUES ('r', 2, 15003, 'chain', 20, 21, 'chain', '127.0.0.1', 80) RETURNING id",
    )
    .fetch_one(&db.pool)
    .await
    .unwrap();
    sqlx::query(
        ...
        VALUES ($1, 0, 20, 15003), ($1, 1, 21, 15004)",
    )
    .bind(rule_id)
    .execute(&db.pool)
    .await
    ...
    sqlx::query(
        ...
        VALUES ($1, 21, 15004, 'tcp', EXTRACT(EPOCH FROM now())::BIGINT + 60, \
    ...
    sqlx::query(
        "INSERT INTO tunnels(id,name,enabled,shared,uid) \
         VALUES(30,'preset',TRUE,TRUE,1)",
    )
    .execute(&db.pool)
    .await
    .unwrap();
    sqlx::query("UPDATE forward_rules SET tunnel_id=30 WHERE id=$1")
        .bind(rule_id)
        .execute(&db.pool)
        .await
        .unwrap();
...
async fn pg_shared_groups_admin_inbound_only() {
    ...
    assert_eq!(
        ...
        vec![10, 13],
        "grant-all excludes historical regular-user-owned groups (PG)"
    );
...
async fn pg_both_groups_are_shared_and_authorized_as_inbound() {
    ...
    sqlx::query(
        "INSERT INTO device_groups \
         (id, name, group_type, token, connect_host, uid) \
         VALUES (13, 'legacy-user-entry', 'in', 't13', '1.2.3.4', 3)",
    )
    .execute(&db.pool)
    .await
    .unwrap();
    ...
    // Preserve coverage for historical invalid rows while current writers
    // reject creating any new ones.
    sqlx::query(
...
#[tokio::test]
async fn pg_checked_user_provisioning_uses_current_registration_settings() {
    let Some(db) = repo("checked_provision").await else {
        return;
    };
    let plan2 = db
...
async fn pg_change_own_password_bumps_version_and_clears_must_change() {
    ...
    assert_eq!(
        db.change_own_password(2, "old", "stale-write")
            ...
            .unwrap(),
        0,
        "a stale verified hash must not overwrite the new password (PG)"
    );
    cleanup(&db).await;
}
...
async fn pg_admin_reset_password_bumps_version_and_sets_must_change() {
    ...
    sqlx::query("UPDATE users SET admin = TRUE WHERE id = 2")
        .execute(&db.pool)
        .await
        .unwrap();
    let n = db
...
#[tokio::test]
async fn pg_buy_plan_quota_overflow_rolls_back() {
    let Some(db) = repo("buy_quota_overflow").await else {
        return;
    };
    let (alice, pid) = seed_buyer_and_plan(&db, "100.00", i64::MAX, "10.00", 0, false).await;
    sqlx::query("UPDATE users SET plan_id = $1, traffic_limit = 1 WHERE id = $2")
        .bind(pid)
        .bind(alice)
        .execute(&db.pool)
        .await
    ...
    let (balance, traffic_limit): (String, i64) =
        sqlx::query_as("SELECT balance, traffic_limit FROM users WHERE id = $1")
            .bind(alice)
            .fetch_one(&db.pool)
            .await
            .unwrap();
...
#[tokio::test]
async fn pg_guarded_buy_rejects_a_plan_changed_after_the_caller_snapshot() {
    let Some(db) = repo("buy_plan_changed").await else {
        return;
    };
    let (alice, pid) = seed_buyer_and_plan(&db, "100.00", 1_000_000, "30.00", 0, false).await;
    sqlx::query("UPDATE plans SET price = '40.00' WHERE id = $1")
        .bind(pid)
        .execute(&db.pool)
        .await
        .unwrap();
    ...
    sqlx::query("UPDATE plans SET price = '30.00' WHERE id = $1")
        .bind(pid)
        .execute(&db.pool)
        .await
        .unwrap();
...
#[tokio::test]
async fn pg_guarded_buy_rejects_historical_unusable_plan_grants_before_charging() {
    let Some(db) = repo("buy_invalid_grant").await else {
        return;
    };
    let (alice, pid) = seed_buyer_and_plan(&db, "100.00", 1_000_000, "30.00", 0, false).await;
    sqlx::query(
        "INSERT INTO device_groups(id,name,group_type,token,uid) \
         VALUES(9901,'legacy-plan-group','in','legacy-plan-token',$1)",
    )
    .bind(alice)
    .execute(&db.pool)
    .await
    .unwrap();
    sqlx::query("INSERT INTO plan_device_groups(plan_id,device_group_id) VALUES($1,9901)")
        .bind(pid)
        .execute(&db.pool)
        .await
        .unwrap();
...
#[tokio::test]
async fn pg_update_plan_fields_rolls_back_scalars_when_grant_replacement_fails() {
    let Some(db) = repo("update_plan_atomic").await else {
        return;
    };
...
#[tokio::test]
async fn pg_update_plan_fields_enforces_type_duration_invariant_atomically() {
    let Some(db) = repo("update_plan_invariant").await else {
        return;
    };
    let data_id = db
        .insert_plan(
...
async fn pg_buy_plan_grant_all_sets_flag() {
    ...
    sqlx::query(
        "INSERT INTO forward_rules \
         ...
         (210,'active',$1,20210,50,'127.0.0.1',80,FALSE,FALSE), \
         (211,'system-paused',$1,20211,50,'127.0.0.1',80,TRUE,TRUE)",
    )
    .bind(alice)
    .execute(&db.pool)
    .await
...
#[tokio::test]
async fn pg_admin_edit_user_plan_adjusts_expiry_without_rewriting_plan_id() {
...
#[tokio::test]
async fn pg_admin_edit_user_plan_skips_admin_users() {
    ...
    sqlx::query("INSERT INTO users (id, username, password, admin) VALUES (90, 'adm', 'x', TRUE)")
        ...
        .unwrap();
    let affected = db
        .admin_edit_user_plan(90, 1, false, Some("2099-12-31 00:00:00"))
        .await
        .unwrap();
...
#[tokio::test]
async fn pg_statistics_upsert_is_idempotent_and_cleanup_is_scoped() {
    let Some(db) = repo("dashboard_stats").await else {
        return;
    };
    db.upsert_stats(
        "dashboard_global",
...
#[tokio::test]
async fn pg_reusable_tunnel_repository_contract() {
    let Some(db) = repo("reusable_tunnel_contract").await else {
        return;
    };
    ...
    sqlx::query(
        "INSERT INTO forward_rules(name,uid,listen_port,protocol,route_mode,forward_mode, \
         ...
         VALUES('preset-rule',1,35001,'tcp','chain','chain',701,702,'203.0.113.8',443,$1)",
    )
    .bind(tunnel_id)
    .execute(&db.pool)
    .await
    ...
    sqlx::query(
        "INSERT INTO forward_rule_targets(rule_id,position,host,port,weight,enabled) \
         VALUES($1,1,'203.0.113.9',8443,2,TRUE)",
    )
    .bind(rule_id)
    .execute(&db.pool)
    .await
    ...
    let exit: i64 = sqlx::query_scalar("SELECT device_group_out FROM forward_rules WHERE id=$1")
        .bind(rule_id)
        .fetch_one(&db.pool)
        .await
        .unwrap();
    ...
    let route_a = [(701, None), (702, Some(36_001)), (703, Some(36_003))];
    let route_b = [(701, None), (702, Some(36_001)), (703, Some(36_004))];
    let db_a = PgRepository::new(db.pool.clone());
    let db_b = PgRepository::new(db.pool.clone());
    let (result_a, result_b) = tokio::join!(
        db_a.update_tunnel_full(
            tunnel_id,
            None,
            None,
            None,
    ...
    assert_eq!(
        outcomes
            .iter()
            .filter(|result| matches!(result, Err(DbError::TunnelUnavailable)))
            .count(),
        1,
        "a stale concurrent path update must be a deterministic conflict"
    );
    ...
    sqlx::query(
        "INSERT INTO forward_rule_hops(rule_id,position,device_group_id,listen_port,tunnel_port) \
         VALUES($1,0,801,36102,NULL),($1,1,802,36103,36104)",
    )
    .bind(rule_id)
    .execute(&db.pool)
    .await
...
async fn active_route_change_leases_old_downstream_ports_and_pause_revokes_them() {
    let Some(db) = repo("route_transition_overlap").await else {
        return;
    };
    for (id, name, group_type, host) in [
    ...
    sqlx::query(
        "INSERT INTO forward_rule_hops(rule_id,position,device_group_id,listen_port,tunnel_port) \
         VALUES($1,0,901,41001,NULL),($1,1,902,41002,41003)",
    )
    .bind(rule_id)
    .execute(&db.pool)
    .await
    ...
    sqlx::query(
        "UPDATE forward_rule_route_transitions SET activate_at=0,expires_at=0 WHERE rule_id=$1",
    )
    .bind(rule_id)
    .execute(&db.pool)
    .await
    ...
    let direct_rule_id: i64 = sqlx::query_scalar(
        "INSERT INTO forward_rules(name,uid,listen_port,protocol,route_mode,forward_mode, \
...
#[tokio::test]
async fn pg_tunnel_entry_move_keeps_a_scoped_drain_and_billing_lease() {
    let Some(db) = repo("tunnel_entry_drain").await else {
        return;
    };
    for (id, name, group_type, host) in [
        (721, "old-entry", "in", ""),
    ...
    let rule_id: i64 = sqlx::query_scalar(
        "INSERT INTO forward_rules(name,uid,listen_port,protocol,route_mode,forward_mode, \
         device_group_in,device_group_out,target_addr,target_port,tunnel_id) \
         VALUES('draining-rule',1,35021,'tcp','chain','chain',721,723,'203.0.113.21',443,$1) \
         RETURNING id",
    )
    .bind(tunnel_id)
    .fetch_one(&db.pool)
    .await
    .unwrap();
    let paused_rule_id: i64 = sqlx::query_scalar(
        "INSERT INTO forward_rules(name,uid,listen_port,protocol,route_mode,forward_mode, \
         ...
         VALUES('paused-before-move',1,35023,'tcp','chain','chain',721,723, \
                '203.0.113.23',443,$1,TRUE) RETURNING id",
    )
    .bind(tunnel_id)
    .fetch_one(&db.pool)
    .await
    .unwrap();
    let newly_shared_rule_id: i64 = sqlx::query_scalar(
        "INSERT INTO forward_rules(name,uid,listen_port,protocol,route_mode,forward_mode, \
         device_group_in,device_group_out,target_addr,target_port,tunnel_id) \
         VALUES('newly-shared-during-move',2,35024,'tcp','chain','chain',721,723, \
                '203.0.113.24',443,$1) RETURNING id",
    )
    .bind(tunnel_id)
    .fetch_one(&db.pool)
    .await
    ...
    let paused_lease_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM forward_rule_retired_entries WHERE rule_id=$1")
            .bind(paused_rule_id)
            .fetch_one(&db.pool)
            .await
            .unwrap();
    assert_eq!(paused_lease_count, 0);
    let newly_shared_lease_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM forward_rule_retired_entries WHERE rule_id=$1")
            ...
            .await
            .unwrap();
    assert_eq!(newly_shared_lease_count, 0);
    let future_rule_id: i64 = sqlx::query_scalar(
        "INSERT INTO forward_rules(name,uid,listen_port,protocol,route_mode,forward_mode, \
         device_group_in,device_group_out,target_addr,target_port,tunnel_id) \
         VALUES('future-rule',1,35022,'tcp','chain','chain',722,723,'203.0.113.22',443,$1) \
         RETURNING id",
    )
    .bind(tunnel_id)
    .fetch_one(&db.pool)
    ...
    sqlx::query("UPDATE forward_rules SET paused=FALSE WHERE id=$1")
        .bind(rule_id)
        .execute(&db.pool)
        .await
        .unwrap();
    sqlx::query("UPDATE forward_rules SET tunnel_id=NULL WHERE id=$1")
        .bind(rule_id)
        .execute(&db.pool)
        .await
        .unwrap();
    ...
    sqlx::query("UPDATE forward_rules SET tunnel_id=$1 WHERE id=$2")
        ...
        .await
        .unwrap();
    sqlx::query("UPDATE tunnels SET enabled=FALSE WHERE id=$1")
        .bind(tunnel_id)
        .execute(&db.pool)
        .await
        .unwrap();
    ...
    sqlx::query("UPDATE tunnels SET enabled=TRUE WHERE id=$1")
        .bind(tunnel_id)
        .execute(&db.pool)
        .await
        .unwrap();
    sqlx::query("UPDATE forward_rule_retired_entries SET expires_at=0 WHERE rule_id=$1")
        .bind(rule_id)
        .execute(&db.pool)
        .await
        .unwrap();
    ...
    // Enabling and moving an already-disabled tunnel must not grant the old
    // entry a drain lease because there was no running listener to drain.
    sqlx::query("UPDATE forward_rules SET paused=FALSE WHERE id=$1")
        .bind(rule_id)
        .execute(&db.pool)
        .await
        .unwrap();
    sqlx::query("DELETE FROM forward_rule_retired_entries WHERE tunnel_id=$1")
        .bind(tunnel_id)
        .execute(&db.pool)
        .await
        .unwrap();
    ...
    let disabled_move_lease_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM forward_rule_retired_entries WHERE tunnel_id=$1")
            .bind(tunnel_id)
            .fetch_one(&db.pool)
            .await
            .unwrap();
...
#[tokio::test]
async fn pg_tunnel_entry_move_uses_previous_state_for_tail_billing_lease() {
    let Some(db) = repo("tunnel_entry_tail_billing").await else {
        return;
    };
    sqlx::query(
    ...
    .await
    .unwrap();
    sqlx::query("INSERT INTO users(id,username,password,admin) VALUES(2,'tail-u2','x',FALSE)")
        .execute(&db.pool)
        .await
        .unwrap();
    ...
    let tunnel_id = db
        ...
        .await
        .unwrap();
    let admin_rule: i64 = sqlx::query_scalar(
        "INSERT INTO forward_rules(name,uid,listen_port,protocol,route_mode,forward_mode, \
         device_group_in,device_group_out,target_addr,target_port,tunnel_id) \
         VALUES('tail-admin',1,35131,'tcp','chain','chain',731,733,'203.0.113.31',443,$1) \
         RETURNING id",
    )
    .bind(tunnel_id)
    .fetch_one(&db.pool)
    .await
    .unwrap();
    let user_rule: i64 = sqlx::query_scalar(
        "INSERT INTO forward_rules(name,uid,listen_port,protocol,route_mode,forward_mode, \
         device_group_in,device_group_out,target_addr,target_port,tunnel_id) \
         VALUES('tail-user',2,35132,'tcp','chain','chain',731,733,'203.0.113.32',443,$1) \
         RETURNING id",
    )
    .bind(tunnel_id)
    .fetch_one(&db.pool)
    ...
    let leased_rules: Vec<i64> = sqlx::query_scalar(
        "SELECT rule_id FROM forward_rule_retired_entries WHERE tunnel_id=$1 AND device_group_id=$2 ORDER BY rule_id",
    )
    .bind(tunnel_id)
    .bind(731_i64)
    .fetch_all(&db.pool)
...
#[tokio::test]
async fn pg_reusable_tunnel_repository_revalidates_group_roles_inside_write() {
    let Some(db) = repo("tunnel_group_revalidation").await else {
        return;
    };
    sqlx::query("INSERT INTO users(id,username,password,admin) VALUES(2,'legacy-owner','x',FALSE)")
        .execute(&db.pool)
...
#[tokio::test]
async fn pg_group_token_revision_and_preset_tunnel_invariants_are_atomic() {
    let Some(db) = repo("group_tunnel_guard").await else {
        return;
    };
    sqlx::query(
        "INSERT INTO device_groups (id,name,group_type,token,uid,connect_host) VALUES \
