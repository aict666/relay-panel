...
预设隧道与共享 listener 从配置协议 v8 开始提供；当前 v9 又加入了路径变更的分阶段切换与旧连接排空。面板与节点严格校验协议版本，因此应在维护窗口协调升级：

1. 备份数据库并记录现有规则；升级不会自动把任何旧规则迁移到预设隧道。
2. 先准备所有节点的新二进制或安装命令，再升级面板和全部 relay-node。
3. 确认节点报告 `config_protocol_version=9`，且节点状态页没有 listener error。
...
旧面板读取带 `tunnel_id` 的规则时看不到规则级 hop；它会停止下发该 `route_mode=chain` 规则，而不会把它误当直连。这个安全失败行为不能替代备份，回滚面板时仍应同步回滚节点，并根据需要恢复数据库。
...
1. 写入两个 `false` 后升级面板。
2. 逐台升级所有 relay-node。
3. 至少等待两个配置轮询周期，确认节点报告当前要求的配置协议版本（本版本为 v9），且没有 listener error。
