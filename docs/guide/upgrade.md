...
预设隧道与共享 listener 从配置协议 v8 开始提供，v9 加入了路径变更的分阶段切换与旧连接排空；当前 v10 又加入公共入口 TCP 的 TLS ClientHello 屏蔽。配置快照要求面板与节点协议匹配，因此应在维护窗口协调升级：

1. 备份数据库并记录现有规则；升级不会自动把任何旧规则迁移到预设隧道。
2. 先准备所有节点的新二进制或安装命令，再升级面板和全部 relay-node。
3. 确认节点报告 `config_protocol_version=10`，且节点状态页没有 listener error。
...
旧面板读取带 `tunnel_id` 的规则时看不到规则级 hop；它会停止下发该 `route_mode=chain` 规则，而不会把它误当直连。启用入口协议屏蔽后，v10 面板会向不兼容节点返回可持久化的空配置，旧节点会停止该组监听；v10 节点写入的带策略离线缓存也不能被旧节点读取，防止降级后忽略策略继续转发。因此推荐先准备并升级节点，再开启策略；如果必须回滚到 v9，应先关闭屏蔽策略并等待节点取得无策略快照。这个安全失败行为不能替代备份，回滚面板时仍应同步回滚节点，并根据需要恢复数据库。
...
1. 写入两个 `false` 后升级面板。
2. 逐台升级所有 relay-node。
3. 至少等待两个配置轮询周期，确认节点报告当前要求的配置协议版本（本版本为 v10），且没有 listener error。
