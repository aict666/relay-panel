...
- 首次安装不再启用公开的 `admin` / `admin123` 凭据。面板会在监听端口前生成
  高熵随机管理员密码，使用 bcrypt 存储并只在成功初始化的进程日志中输出一次；
  仍使用历史默认密码的旧安装会在升级启动时自动轮换，已修改的密码保持不变。
- 自定义多跳规则现在对入口、中继和出口的每一个 hop 执行所有者套餐授权校验，
  并在 API、服务、SQLite/PostgreSQL 写事务和节点配置下发阶段重复复核；存量越权
  链会立即停止下发，管理员托管的共享预设隧道继续采用入口授权模型。

### Documentation

- 更新首次登录、容器日志取密、升级兼容、自定义多跳授权边界和部署验证说明；
  CI 与端到端测试也改为读取实际生成的一次性密码。

## [1.3.12] - 2026-07-22

### Added

- 隧道管理新增端到端诊断入口：管理员可通过隧道上一条正在运行的 TCP 规则复用
  现有认证探针检查共享监听、逐跳路由与最终目标；诊断结果使用隧道名称，不再向
  操作界面展示仅供协议关联使用的隧道数字 ID。
...
- 全面收敛前端常驻说明文案：规则、设备组、套餐、用户、系统设置、商店和仪表盘
  改为由字段名、占位符、禁用/隐藏状态和操作按钮直接表达，只保留风险警告与按需
  查看提示。
- 入口协议屏蔽改为 HTTP/TLS 复选框，并统一使用 `blocked_protocols` 数组作为表单值；
  加载失败页面提供直接刷新操作，不再重复展示说明性文本。
...
- **入口设备组可阻断原始 TCP 上的 TLS ClientHello。** 管理员可按设备组启用
  TLS 屏蔽；节点仅检查公共入口，识别失败时放行，避免影响出口、中间隧道、
  UDP/QUIC、STARTTLS 和服务端先发协议。
- **启用屏蔽策略时，旧协议节点安全停止接流量。** 配置协议不兼容的节点会从
  HTTP 轮询取得并持久化空配置，避免 WebSocket 中断或进程重启后继续使用旧的
  未过滤监听配置；v10 的策略缓存也采用协议绑定格式，防止二进制降级后旧节点
  忽略新字段并离线恢复未过滤监听。协议 v4–v7 节点在令牌被轮换或删除后也会
  收到兼容空配置，不再把 401 当作临时故障而保留旧监听。

### Added

- 设备组、共享线路和节点状态接口新增屏蔽协议与累计拦截指标；管理页面提供 TLS
  开关、入口标签和在线节点拦截汇总，并记录策略变更审计。
- SQLite 与 PostgreSQL 新增 `blocked_protocols` JSON 数组迁移，未知协议会被
  API 拒绝，组改为出口或监控用途时自动清空策略。

### Changed

- 节点配置协议升级到 v10。策略只下发到公共入口原始 TCP listener，开关变化
  热重载 accept loop，已有连接继续排空。

### Fixed

- 分阶段变更同时修改入口端口或传输方式与协议策略时，节点会先重建旧代际，再
  准备新代际，不会提前启用新路由或拆分 `tcp_udp` 两个半边的代际。

### Tests

- 增加 TLS 分片/超时/fail-open、流量统计与 TFO 回归，以及设备组 API、数据库
  迁移、旧节点持久化空配置、共享线路展示和跨端口 staged route 回归测试。
